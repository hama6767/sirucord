...
- 人が1人だけなら「汁が開始されました！集合汁！」の一文だけを投稿。配信の有無にかかわらず、画像・名前・説明は付けない。
- 2人以上の場合は、全参加者のアイコン・名前・配信中バッジ・プレイ中のゲーム名をカード画像にして添付。配信者にはネオンの発光枠、オーラ、光の粒、稲妻のエフェクトを表示。名前・ゲーム名は読みやすく保ち、人数に合わせて折り返します。
- 投稿にDiscordへの参加URLを付けない。
- 配信者の表示名、ゲーム名、ゲーム側が公開する詳細・状態・パーティー人数、チャンネル名と通話参加人数を自動投稿。日々の手入力や配信者PCへの追加アプリは不要。
- 参加者アイコンはサーバー専用アバターを優先し、通常のアバター・Discord標準アイコンの順で取得。
- 指定したテキストチャンネルの最新の新規投稿・編集を、通話に人がいる場合だけ共有。動画・ページのURLには取得できたタイトルを追加。
- どこかに人がいれば5分ごと、すべて無人なら30分ごとに確認。情報が変わった場合だけ追加投稿し、経過時間だけでは投稿しない。
...
| 通知の速さ | 無人時は30分間隔、参加を確認してからは5分間隔。Actionsの混雑でさらに遅延・欠落する場合あり |
...
| 画像 | 2人以上のときは参加者のアイコン・配信状況・ゲーム名のカード。**Discord映像の自動撮影は非対応** |
...
2. 自動取得用の **Presence Intent** を有効にします。`share_channel_id` を設定する場合は **Message Content Intent** も有効にし、そのテキストチャンネルの **View Channel / Read Message History** を許可します。Server Members Intentは不要です。
...
`use_activity = true` が自動取得モードです。説明用テキストチャンネルは読み取りません。2人以上の場合にゲーム名・詳細も取得し、公開アクティビティがなければ取得できなかった旨を投稿します。画像は参加者アイコンを優先し、手動画像転送用の `screenshots` 設定とは独立しています。
...
1人だけの状態では配信の開始・終了やゲーム変更だけで同じ一文を再投稿しません。2人以上のアイコン画像は1枚にまとめ（最大256人）、投稿が必要な場合だけCDNから取得します。取得できないアイコンは灰色の人型を表示し、画像の代替テキストに参加者名を並べます。参加者の表示順はユーザーID順で固定します。

画像内の配信バッジはGo Liveの状態、ゲーム名はその参加者の公開アクティビティを使います。取得できないゲーム名は空欄です。日本語フォントを実行ファイルに同梱するため、実行時のフォント取得・ブラウザ・配信者PCの追加アプリは必要ありません。

### 通話中のチャンネル共有

`[[servers]]` に `share_channel_id = "567890123456789012"` を追加すると、そのサーバーの監視対象ボイスチャンネルに人がいる場合だけ、指定テキストチャンネルの最新1件を読み取ります。初回は最新メッセージを基準として保存し、過去の投稿をまとめて転送しません。その後の新規投稿・最新投稿の編集を、1回の監視につき最大1件共有します。同じ内容の再共有や、削除によって現れた古い投稿の転送はしません。複数人が配信していても共有はサーバーにつき1回です。

共有をOFFにする場合は `share_channel_id` の行を削除し、Actions Secret `SIRUCORD_CONFIG` も更新します。OFF時は新しいメッセージの取得・共有と、送信待ちの再送も行いません。

本文は最大180文字、リンク・添付ファイルへのリンクは最大2件を共有します。URLのタイトルはDiscordの埋め込み情報を優先し、情報がなければYouTube/VimeoのoEmbedや公開ページのOpen Graph・HTMLタイトルを取得します。非公開ページや取得失敗の場合も本文とURLは共有します。添付ファイル自体の再アップロードは行いません。リンク先へのリクエストにDiscord/Mastodonのトークンは付けず、非公開ネットワークへのアクセスや無制限のダウンロードも防ぎます。

配信通知の最短30分の追加投稿間隔とは独立して、新しいチャンネル投稿は次の正常な監視時に共有します。無人時はこのテキストチャンネルを読み取りません。初回基準・共有済みメッセージ・送信予定は既存の暗号化履歴に保存し、投稿の再試行には同じ重複防止キーを使います。共有は通常の在室通知とは別投稿なので、1人在室の通知本文は指定の一文のままです。
...
`GITHUB_TOKEN` はActionsが自動発行します。個人用GitHubトークンの登録は不要です。履歴保存の `contents: write` と、追加監視を切り替える `actions: write` をワークフローに設定しています。組織ポリシーやブランチルールで拒否される場合は管理者による設定が必要です。
...
基本監視の `Announce Discord streams` は毎時14分・44分（48回/日）です。どこかの対象チャンネルに人がいることを確認すると、`Active Discord checks` を有効にして残りの5分刻みの確認を追加します。すべて無人なら追加監視自体を無効にします。1日中人がいる場合は合計288回/日です。無人から最初の参加を検知するまでには最大約30分とActionsの遅延があり、その後に5分間隔へ移ります。Botだけなら無人扱いです。
...
情報の変化がなければ投稿・画像取得・通知履歴の書き込みは行いません。確認が5分間隔でも、追加投稿の最短間隔は `activity_update_interval_minutes`（既定30分）のままです。定期確認のたびにテストやビルドを実行することもありません。

同じチャンネルでも別々の監視対象チャンネルでも、複数人の配信をそれぞれ投稿対象にします。配信者ごとに履歴と追加投稿間隔を管理し、1人への投稿が他の配信者の新規投稿を抑制することはありません。

非公開リポジトリではActionsの各ジョブが最低1分として計上されます。30日間ずっと無人なら監視は約1,440分、ずっと人がいるなら約8,640分となり、後者はGitHub Freeの月2,000分の無料枠を超えます。開発用ビルドや他の非公開リポジトリの使用量も別途加わります。
...
ライセンス: コードはMIT。埋め込みのNoto Sans JPフォントはSIL Open Font License 1.1（`assets/fonts/OFL.txt`、配布ファイル内の `font-license` を参照）。
